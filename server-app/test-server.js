...
// Import the express app or spawn it. We can require it, but we want to test it running on a port.
// Since index.js starts listening on port 5001 immediately when required, we can require it to start the server!
// Let's first clean up any existing mock voter to make the test repeatable.

async function cleanupAndSetup() {
  const client = await pool.connect();
  try {
    // Delete test votes and voters
    await client.query('DELETE FROM "Votes" WHERE handled_by_server = \'server_1\'');
    await client.query('DELETE FROM "Voters" WHERE identifier = \'TEST_STUDENT_001\'');

    // Insert a fresh test voter
    const res = await client.query(`
      INSERT INTO "Voters" (voter_type, name, identifier, device_fingerprint)
      VALUES ('student', 'Test User', 'TEST_STUDENT_001', 'test_fingerprint_abc')
      RETURNING id
    `);
...
async function runTests() {
  ...
  // Require index.js to start the server on port 5001
  require("./index.js");
  ...
  if (voteRes.statusCode === 200 && voteRes.body.success === true) {
